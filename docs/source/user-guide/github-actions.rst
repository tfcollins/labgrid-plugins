...
.. warning::
   ...
   ``matlab-hw-request.yml``) pinned at ``@v3.4`` (current release).  Removal of the
...
.. list-table::
   ...
   * - ``hw-matrix.yml`` (v1) — **deprecated**
     ...
     - Do not adopt for new consumers; use ``hw-request.yml@v3.4`` instead.
...
.. code-block:: yaml
   ...
   jobs:
     hw:
       uses: tfcollins/labgrid-plugins/.github/workflows/hw-request.yml@v3.4  # bump when a new release tags
       with:
         coordinator: ${{ vars.ADI_LG_COORDINATOR }}
         test-root: "test"
         install-cmd: >-
           uv pip install --quiet --python "$VENV_DIR/bin/python" -e "."
           "adi-labgrid-plugins @ git+https://github.com/tfcollins/labgrid-plugins@v3.4"
...
.. code-block:: yaml
   ...
   jobs:
     hw-matlab:
       uses: tfcollins/labgrid-plugins/.github/workflows/matlab-hw-request.yml@v3.4  # bump when a new release tags
       with:
...
.. code-block:: yaml
   ...
   jobs:
     noos-hw:
       uses: tfcollins/labgrid-plugins/.github/workflows/noos-hw-request.yml@v3.4  # bump when a new release tags
       with:
         coordinator: ${{ vars.ADI_LG_COORDINATOR }}
...
.. code-block:: yaml
   ...
   - uses: tfcollins/labgrid-plugins/.github/actions/setup-uv-venv@v3.4  # bump when a new release tags
     with:
...
.. code-block:: yaml
   ...
   - uses: tfcollins/labgrid-plugins/.github/actions/acquire-place@v3.4  # bump when a new release tags
     with:
