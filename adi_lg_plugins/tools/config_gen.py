...
class ConfigGenerator:
    ...
    def configure_power(self):
        ...
        if driver_type == "VesyncPowerDriver":
            ...
            self.add_resource("VesyncOutlet", {
            ...
            self.add_resource("CyberPowerOutlet", {
                "address": address,
                "outlet": outlet,
                "delay": delay
            })
    ...
    def configure_shell(self):
        ...
        selected_port = Prompt.ask("Select Serial Port", choices=port_choices, default=ports[0] if ports else "Manual Entry")
        ...
        self.add_driver("ADIShellDriver", {
            "prompt": prompt_regex,
    ...
    def configure_sdmux(self):
        ...
        mux_path = Prompt.ask("SD Mux Device Path", default="/dev/disk/by-id/usb-Linux_Autobuild_SD_Mux_...")
    ...
    def configure_mass_storage(self):
        ...
        device = Prompt.ask("Block Device Path (host)", default="/dev/disk/by-id/usb-Linux_Autobuild_SD_Mux_...")
        partition = IntPrompt.ask("Partition Number", default=1)
        
        self.add_resource("MassStorageDevice", {
            "device": device,
            "partition": partition
        })
    ...
    def configure_kuiper(self):
        ...
        self.add_resource("KuiperRelease", {
            "release": release,
            "cache_dir": cache_dir
        })
    ...
    def configure_tftp(self):
        ...
        if use_managed:
            ...
            self.add_resource("TFTPServerResource", {
                "address": address,
                "port": port,
                "root": root
            })
            ...
            console.print("[yellow]Note: BootFPGASoCTFTP strategy expects a managed driver. Adding it anyway.[/yellow]")
            self.add_driver("TFTPServerDriver")
    ...
    def run(self):
        ...
        if selected_strategy == "BootFPGASoC":
            ...
            if Confirm.ask("Configure SSH Driver?", default=False):
                ...
                address = Prompt.ask("Target IP Address (or auto-discovered)", default="192.168.1.10")
