...
class MassStorageDriver(Driver):
    ...
    def __attrs_post_init__(self):
        ...
        for tool in ['pmount', 'pumount']:
            if not shutil.which(tool):
                raise RuntimeError(f"{tool} not found in system PATH. Please install it to use MassStorageDriver.")
    ...
    def unmount_partition(self):
        ...
        try:
            ...
            self.logger.debug(f"Unmounted lg_mass_storage successfully")
        except subprocess.CalledProcessError as e:
