...
@attr.s(eq=False)
class ADIShellDriver(CommandMixin, Driver, CommandProtocol, FileTransferProtocol):
    """ADIShellDriver - Driver to execute commands on the shell
    ADIShellDriver binds on top of a ConsoleProtocol.

    On activation, the ADIShellDriver will look for the login prompt on the console,
    ...
    #     """Wrapper around console.expect"""
    #     return self.console.expect(pattern, timeout=timeout)
