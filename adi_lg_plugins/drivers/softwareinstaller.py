...
class SoftwareInstallerDriver(Driver):
    ...
    def _detect_package_manager(self):
        ...
        raise Exception("No supported package manager found on target (checked apt-get, dnf, opkg, pacman, apk)")
    ...
    def install_package(self, package_name, update=False):
        ...
        if exit_code != 0:
            raise Exception(f"Failed to install package '{package_name}'. Exit code: {exit_code}. Stderr: {stderr}")
        return stdout
    ...
    def clone_repo(self, repo_url, destination, branch=None):
        ...
        stdout, stderr, exit_code = self.command.run(cmd)
        if exit_code != 0:
            raise Exception(f"Failed to clone repo '{repo_url}'. Exit code: {exit_code}. Stderr: {stderr}")
        return stdout
    ...
    def copy_directory(self, local_path, remote_path):
        ...
        try:
            ...
            if exit_code != 0:
                 raise Exception(f"Failed to extract directory. Exit code: {exit_code}. Stderr: {stderr}")
    ...
    def run_build(self, command, directory):
        ...
        stdout, stderr, exit_code = self.command.run(full_cmd, timeout=3600) # Long timeout for builds
        if exit_code != 0:
